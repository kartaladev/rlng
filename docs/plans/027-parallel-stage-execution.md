...
**Architecture:** Consolidate `NewPipeline` onto a `(stages []Stage, opts ...PipelineOption)` signature; move `WithRuleset` from a fluent method to an option; add `WithConcurrency()` / `WithMaxParallel(n)`. `Run` branches: sequential (default, unchanged) or wave-based level-barrier parallel. Errors are selected topo-min (identical to sequential); `ctx` cancellation takes precedence; reported `stageOrder` is reconstructed to topo order. Concurrency threads to the config path via `config.BuildOption`s and to the convenience constructors via `rlng.Option`s. Per-stage read isolation in `Scope`: under concurrency, `Snapshot` deep-copies the map spine so no stage holds a live reference into a map a sibling may mutate (the delivery-gate correction — see the note below).

> **Delivery-gate correction:** the original plan claimed "No `Scope` structural change (existing `s.mu` + Spec-002 namespace isolation make it race-free)." That was **wrong** — the whole-branch `/code-review` and a `-race` regression test (`TestPipelineConcurrencyNoSharedNestedMapRace`) exposed a nested-map read/write race, because a stage's output path (via `WithOutput`) can descend into a pre-existing seed/earlier-level sub-map that a sibling reads live through `Snapshot`. Fixed by marking the `Scope` concurrent and deep-copying `Snapshot`'s eval env. See ADR-0052 §5 (amended) + Correction, and Spec 027 "Concurrency safety (per-stage read isolation)".
...
**Spec coverage:** D1 (determinism) → Task 2 (topo-min, reorderStages) + example. D2 (API consolidation) → Tasks 1, 2. D3 (bound) → Task 2. D4 (wave scheduling) → Task 2 (`computeLevels`/`runWaves`). D5 (error path) → Task 2 (`topoMinError`, dep-failed pruning via stop-after-failing-wave). D6 (cancellation + reported order) → Task 2 (`runWaves` ctx checks, `reorderStages`). Concurrency-safety (per-stage read isolation) → Task 2 locks + concurrent-mode deep-copying `Snapshot` (delivery-gate correction); `-race` gate incl. `TestPipelineConcurrencyNoSharedNestedMapRace`. Success criteria 1–12 → criteria 1,10 (Task 1 migration green); 2–9 (Task 2 tests); 11 (Task 4); 3–4 config (Task 3); 12 example (Task 5). Threading (D2 engine options) → Tasks 3, 4.
